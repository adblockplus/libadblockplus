...
function fetch(url)
{
  ...
  return new Promise((resolve, reject) =>
  {
    ...
    let handleLoad = () =>
    {
      ...
      let response = {
        status,
        // Note: text() is supposed to return a promise, but a promise would
        // get fulfilled later, which would cause our tests to fail. In order
        // to minimize the integration changes, we return a fake promise that
        // gets fulfilled immediately. This also matches with the way
        // XMLHttpRequest works. It may break something on the adblockpluscore
        // side in the future though; therefore, we must pay attention during
        // dependency updates.
        text: () => ({
          then: callback => callback(responseText)
        })
      };
