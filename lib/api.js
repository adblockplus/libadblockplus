...
let API = (() =>
{
  ...
  return {
    ...
    {
      if (Prefs.filter_engine_enabled && subscription instanceof DownloadableSubscription)
        synchronizer.execute(subscription);
    },
    ...
    isFilterEngineEnabled()
    {
      return Prefs.filter_engine_enabled;
    },

    setFilterEngineEnabled(value)
    {
      if (Prefs.filter_engine_enabled === value)
        return;

      Prefs.filter_engine_enabled = value;
    ...
    isAASubscriptionEnabled()
    {
      ...
      if (!Prefs.filter_engine_enabled && Prefs.first_run_subscription_auto_select
          && !filterStorage.initialized)
        return true;
    ...
    checkFilterMatch(url, contentTypeMask, documentUrl, siteKey, specificOnly)
    {
      ...
      // _allowing property is private, but we use it until a proper API is
      // available for looking up allowing filters first.
      return defaultMatcher._allowing.match(urlInfo, contentTypeMask,
