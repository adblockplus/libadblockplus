...
let API = (() =>
{
  ...
  return {
    ...
    checkFilterMatch(url, contentTypeMask, documentUrl, siteKey, specificOnly)
    {
      ...
      let foundFilters = defaultMatcher.search(
        url, contentTypeMask, documentHost, thirdParty, siteKey, specificOnly);
      return foundFilters.whitelist.length > 0 ? foundFilters.whitelist[0] : (foundFilters.blocking.length > 0 ? foundFilters.blocking[0]: null);
    },
