...
let {Downloader, Downloadable} = require("downloader");
let {MILLIS_IN_HOUR} = require("time");
let {compareVersions} = require("versions");
...
let downloader = new Downloader(getDownloadables);
downloader.scheduleChecks(CHECK_INTERVAL, INITIAL_DELAY);
                           ...
                           redirectCallback)
{
  ...
  try
  {
    ...
    if (updateInfo && "version" in updateInfo && "url" in updateInfo &&
        compareVersions(updateInfo.version, _appInfo.version) > 0)
    {
...
function onDownloadError(downloadable, downloadURL, error, responseStatus,
                         redirectCallback)
{
