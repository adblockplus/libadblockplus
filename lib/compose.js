...
{
  if (isValidString(id))
    selectors.add("#" + Utils.escapeCSS(id));
}
...
function composeForClasses(classes, selectors)
{
  ...
  if (cls.length > 0)
    selectors.add(cls.map(c => "." + Utils.escapeCSS(c)).join(""));
}

function composeForSrc(src, tagName, selectors)
{
  if (isValidString(src))
    selectors.add(Utils.escapeCSS(tagName) + "[src=" + Utils.quoteCSS(src) + "]");
}

function composeForStyle(style, tagName, selectors)
{
  if (isValidString(style))
    selectors.add(Utils.escapeCSS(tagName) + "[style=" + Utils.quoteCSS(style) + "]");
}
